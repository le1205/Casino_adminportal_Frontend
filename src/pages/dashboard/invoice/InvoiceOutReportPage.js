...
// sections
import { InvoiceAmountTableToolbar, InvoiceWithdrawTableRow } from '../../../sections/@dashboard/invoice/list';
// api
import { apiWithPostData } from '../../../utils/api';
// url
import {  getWithdrawListUrl, subAcceptUrl, subCancelUrl } from '../../../utils/urlList';
// ----------------------------------------------------------------------
...
export default function InvoiceOutReportPage() {
  const {
    dense,
    page,
    order,
  ...
  const withdrawList = () => {
    try {
      setIsLoading(true);
      setPage(0);
      const url = getWithdrawListUrl + loginUser._id;
      const headers = {};
      const data = {};
      apiWithPostData(url, data, headers).then((response) => {
        setIsLoading(false);
